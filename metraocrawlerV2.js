...
var labelDbC = 'LCeq';
var labelDbA = 'LAeq';
var labelCalcC = 'calc C';
var labelNormC = 'Norm C';

var d = new Date().toISOString().slice(0,17).replace(/-/g,"").replace(/:/g,"").replace("T","-");
var outfile = `output/metraocrawlerv2-${d}.csv`;
...
function getMeasure(onetwothree) {
    ...
    if (headerHelper) {
        headerHelper = false;
        // write header once
        fs.outputFile(outfile, 'measurepoint,timestamp,dBA,dBC,norm,winddir,windspeed,temp,n, calcC\n', function (err) {
            if (err) throw err;
        });
    }
    ...
    .then(function ($) {
        ...
        measures.map(function() {
            ...
            var decibels = $(this).find('table tr td').toArray();

            var dBA = '', dBC = '', calcC= '', norm= '';

            decibels.forEach(element => {
                var value = $(element).find('h10').text();
                var label = $(element).find('h11').text();
                if (value !== '') {
                    switch(label) {
                        case (labelDbC) : 
                            dBC = value;
            ...
            fs.appendFile(outfile, `${measurepoint},${timestamp},${dBA},${dBC},${norm},${closingcolumns}, ${calcC}\n`, function (err) {
            });
            return $(this).text();
        });
